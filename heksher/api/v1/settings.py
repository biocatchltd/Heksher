...
from heksher.api.v1.setting_declaration import declare_setting, declare_setting_enpoint_args
...
from heksher.db_logic.rule import (
    db_get_actual_configurable_features, db_get_rules_feature_values, db_get_rules_for_setting
)
from heksher.db_logic.setting import (
    db_bump_setting_version, db_delete_setting, db_get_canonical_names, db_get_setting, db_get_settings,
    db_rename_setting, db_set_setting_type
)
from heksher.db_logic.setting_configurable_features import db_set_settings_configurable_features
...
router.add_api_route('/declare', declare_setting, **declare_setting_enpoint_args)
...
async def delete_setting(name: str, app: HeksherApp = application):
    ...
    async with app.engine.begin() as conn:
        ...
        deleted = await db_delete_setting(conn, setting.name)
        if not deleted:
            return PlainTextResponse('setting name not found', status_code=status.HTTP_404_NOT_FOUND)
...
async def set_setting_type(name: str, input: PutSettingTypeInput, app: HeksherApp = application):
    ...
    async with app.engine.begin() as conn:  # note that the connection may be upgraded to transaction
        ...
        if input.type == setting.type:
            ...
            async with app.engine.begin() as conn:
                await db_bump_setting_version(conn, name, input.version)
            return Response(status_code=status.HTTP_204_NO_CONTENT)
        conflicts = []
        ...
        rules = await db_get_rules_for_setting(conn, setting.name)
        bad_rules = {rule_id: rule_value for (rule_id, rule_value) in rules if not input.type.validate(rule_value)}
        if bad_rules:
            conditions = await db_get_rules_feature_values(conn, list(bad_rules.keys()))
        ...
        if conflicts:
            return JSONResponse(PutSettingTypeConflictOutput(conflicts=conflicts).dict(),
                                status_code=status.HTTP_409_CONFLICT)
        async with app.engine.begin() as conn:
            await db_set_setting_type(conn, setting.name, input.type, input.version)
        return None
...
async def rename_setting(name: str, input: RenameSettingInput, app: HeksherApp = application):
    ...
    async with app.engine.begin() as conn:  # note that the connection may be upgraded to transaction
        ...
        setting = await db_get_setting(conn, canonical_name, include_metadata=False, include_aliases=False,
                                       include_configurable_features=False)
        existing_version = parse_setting_version(setting.version)
        new_version = parse_setting_version(input.version)
        ...
        if input.name == canonical_name:
            ...
            async with app.engine.begin() as conn:
                await db_bump_setting_version(conn, canonical_name, input.version)
            return Response(status_code=status.HTTP_204_NO_CONTENT)
        ...
        if names_map[input.name] is not None:
            ...
            # otherwise - the operation cannot be done since the new name already exists as another setting's name or
            # alias
            if names_map[input.name] != canonical_name:
                return PlainTextResponse('name already exists', status_code=status.HTTP_409_CONFLICT)
        async with app.engine.begin() as conn:
            await db_rename_setting(conn, canonical_name, input.name, input.version)
    return None
...
async def set_configurable_features(name: str, input: ConfigurableFeaturesInput, app: HeksherApp = application):
    async with app.engine.begin() as conn:
        ...
        if removed_cfs:
            ...
            actual_cfs_in_use = await db_get_actual_configurable_features(conn, setting.name)
        ...
        if not (existing_cfs > new_cfs) and existing_version[0] == new_version[0]:
            ...
            return PlainTextResponse('Cannot add new configurable features on a minor version bump',
                                     status_code=status.HTTP_409_CONFLICT)
        async with app.engine.begin() as conn:
            await db_set_settings_configurable_features(conn, setting.name, input.configurable_features, input.version)
    return Response(status_code=status.HTTP_204_NO_CONTENT)
